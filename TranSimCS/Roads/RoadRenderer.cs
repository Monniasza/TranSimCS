...
namespace TranSimCS.Roads {
    ...
    public static class RoadRenderer {
        ...
        public static void GenerateLaneStripMesh(LaneStrip laneStrip, MultiMesh renderer, float voffset = 0.01f) {
            ...
            GenerateLaneRangeMesh(tag, roadBin, laneStrip.Spec.Color, voffset, laneStrip); // Generate the lane tag mesh

            //Generate arrows
            float aoffset = 0.15f;
            var t = 0.5f;
            var splines = GenerateSplines(tag, voffset); // Generate the splines for the left and right lanes
