...
namespace TranSimCS.Spatial {
    public interface IBVHElement {
        ...
        public bool ComputeIntersection(Ray ray, out float distance);
    }
    public sealed class MeshBvh<T> where T : IBVHElement {
        ...
        public struct Triangle : IBVHElement{
            ...
            public bool ComputeIntersection(Ray ray, out float distance) {
                ...
                var intersection = GeometryUtils.RayIntersectsTriangle(ray, A, B, C, out distance);
                return intersection;
            }
        }

        private MeshBvh(List<T> objects) {
            this.objects = objects.ToArray();
            var nodesList = new List<Node>();
            if (objects.Count > 0)
                BuildRecursive(0, objects.Count, nodesList);
            nodes = nodesList.ToArray();
        }

        public BoundingBox Bounds => nodes.Length == 0 ? default : nodes[0].Bounds;

        private int BuildRecursive(int start, int count, List<Node> nodes) {
            ...
            int left = BuildRecursive(start, mid - start, nodes);
            int right = BuildRecursive(mid, start + count - mid, nodes);
        ...
        private BoundingBox ComputeBounds(int start, int count) {
            ...
            for (int i = 1; i < count; i++)
                bounds = BoundingBox.CreateMerged(bounds, objects[start + i].Bounds);
            return bounds;
        }

        private BoundingBox ComputeCentroidBounds(int start, int count) {
            Vector3 min = objects[start].Bounds.Min, max = min;
            for (int i = 1; i < count; i++) {
                var c = objects[start + i].Bounds.Min;
        ...
        private static void Sort(int start, int count, int axis) {
            // Implement sorting based on the selected axis.
        }

        public bool RayIntersect(Ray ray, out float distance) {
            ...
            while (stackSize > 0) {
                ...
                if (node.IsLeaf) {
                    for (int i = 0; i < node.Count; i++) {
                        ...
                        if (objects[objIndex].ComputeIntersection(ray, out float dist) && dist < distance) {
                            distance = dist;
                        }
                    }
                } else {
